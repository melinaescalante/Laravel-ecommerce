...
<div class="container">
    ...
    <div class="row margin">
        @if ($purchasePendant)
                            ...
                            <tr>
                                ...
                                <td class="border  px-4 py-2 text-center">
                                    <div class="d-flex gap-1  flex-wrap">
                                        <form action="{{ route('games.add.cart', ['id' => $item['game']->id_game]) }}"
                                            method="post">
                                            @csrf
            ...
            <form action="{{route('test.mercadopago.show.v2')}}" method="post">
                @csrf
                <input type="hidden" name="items" value="{{ json_encode($gamesWithQuantities) }}">
                <input type="hidden" name="purchase" value="{{ $purchasePendant}}">
                <button class="btn boton mt-3 " style="width:100%">Ir a pagar</button>
            </form>


        @else
        <div class="mt-5 d-flex flex-column align-items-center my-auto gap-2">

            <h2 class="text-center ">Tu carrito está vacío</h2>
            <p>¡Comienza a comprar!</p>
            <a href="{{route('games')}}" class="btn boton">Navegar</a>
        </div>
        @endif
    </div>
</div>
